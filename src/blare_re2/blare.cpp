...
int MultiMatchSingle_CountAll (const std::string & line, std::vector<std::shared_ptr<RE2>> & c_regs, std::shared_ptr<RE2> & reg0, const std::vector<std::string> prefixes, const std::vector<std::string> & regs, bool prefix_first, std::vector<size_t> & prev_prefix_pos) {
    std::string sm;
...
int SplitMatchSingle_CountAll (const std::string & line, RE2 & reg, const std::tuple<std::string, std::string, std::string> & r) {
    int count = 0;
    std::string sm;
...
int FullMatchSingle_CountAll (const std::string & line, RE2 & reg) {
    std::string sm;
...
std::tuple<double, int, unsigned int> BlareCountAllRe2 (const std::vector<std::string> & lines, std::string reg_string) {
    ...
    for (; idx < skip_size; idx++) {
        switch(dist(gen)) {
            case 0: count += SplitMatchSingle_CountAll(lines[idx], reg_suffix, r); break;
            case 1: count += MultiMatchSingle_CountAll(lines[idx], c_regs, reg0, prefixes, regs, prefix_first, prev_prefix_pos); break;
            case 2: count += FullMatchSingle_CountAll(lines[idx], reg_full); break;
        }
    }
